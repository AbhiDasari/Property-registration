...
installChaincode() {
  ...
  set -x
  peer chaincode install -n propnet -v "${VERSION}" -l "${LANGUAGE}" -p "${CC_SRC_PATH}" >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "(propnet) Chaincode installation on peer${PEER}.${ORG} has failed"
  echo "===================== (propnet) Chaincode is installed on peer${PEER}.${ORG} ===================== "
  echo
}
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode instantiate -o orderer.property-registration-network.com:7050 -C "$CHANNEL_NAME" -n "${NAME}" -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.property-registration-network.'"${CONTRACT}"':instantiate"]}' -P "OR ('${ORG}MSP.member')" >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "${NAME}-${CONTRACT} Chaincode instantiation on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' failed"
    echo "===================== ${NAME}-${CONTRACT} Chaincode is instantiated on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' ===================== "
    echo

  else

    set -x
    peer chaincode instantiate -o orderer.property-registration-network.com:7050 --tls "$CORE_PEER_TLS_ENABLED" --cafile $ORDERER_CA -C $CHANNEL_NAME -n "${NAME}" -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.property-registration-network.'"${CONTRACT}"':instantiate"]}' -P "OR ('${ORG}MSP.member')" >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "${NAME}-${CONTRACT} Chaincode instantiation on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' failed"
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode upgrade -o orderer.property-registration-network.com:7050 -C $CHANNEL_NAME -n "${NAME}" -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.property-registration-network.'"${CONTRACT}"':instantiate"]}' -P "OR ('${ORG}MSP.member')" >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "${NAME}-${CONTRACT} Chaincode instantiation on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' failed"
    ...
    set -x
    peer chaincode upgrade -o orderer.property-registration-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n "${NAME}" -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.property-registration-network.'"${CONTRACT}"':instantiate"]}' -P "OR ('${ORG}MSP.member')" >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "${NAME}-${CONTRACT} Chaincode instantiation on peer${PEER}.${ORG} on channel '$CHANNEL_NAME' failed"
